    ...
    data: {
      ...
      quotation_request_line_items: {
        create: Array.from({ length: 3 }).map(() => ({
          ...
          quotation_request_line_item_quantity: faker.number.int({ min: 1, max: 10 }),
        })),
      },
    },
...
// Seed Supplier Quotations
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const lineItem = faker.helpers.arrayElement(quotationRequest.quotation_request_line_items);
