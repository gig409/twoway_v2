...
export const FormSchema = z.object({
    ...
    product_category_attributes: z.array(z.object({
...
export default function ProductCategoryForm({ isEditing, actionData, product_category }: ProductCategoryFormProps) {
  ...
  const defualtAttributes = Object.entries(attributes).map(([key, value]) => ({ key, value }));
  ...
  const [form, fields] = useForm({
    ...
    defaultValue: isEditing && product_category ? {
                    ...
                    <FieldGroup>
                        <Field className="text-left">
                                ...
                                return (
                                    <Fieldset key={field.key}>
                                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                            <Field>
                                                <Label htmlFor={attributeFields.key.id}>Attribute Key</Label>
                                                <Input {...getInputProps(attributeFields.key, {type: 'text'})} invalid={!!attributeFields.key.errors} placeholder="Enter attribute key" />
                                                {attributeFields.key.errors && <ErrorMessage>{attributeFields.key.errors}</ErrorMessage>}
                                            </Field>
                                            <Field>
                                                <Label htmlFor={attributeFields.value.id}>Attribute Value</Label>
                                                <Input {...getInputProps(attributeFields.value, {type: 'text'})} invalid={!!attributeFields.value.errors} placeholder="Enter attribute value" />
                                                {attributeFields.value.errors && <ErrorMessage>{attributeFields.value.errors}</ErrorMessage>}
                                            </Field>
                                        </div>
                                        <Button type='button' onClick={() => form.remove({ name: fields.product_category_attributes.name, index })} disabled={attrubutesList.length === 0}>
                                            Remove Attribute
                                        </Button>
                                    </Fieldset>
                                )
                            ...
                            {fields.product_category_attributes.errors && (
                                <ErrorMessage>{fields.product_category_attributes.errors}</ErrorMessage>
                            )}
                        </Field>
                    </FieldGroup>
                    <Button type='button' onClick={() => form.insert({ name: fields.product_category_attributes.name })}>
                        Add Attribute
                    </Button>
                </FieldGroup>
                {form.errors && (
